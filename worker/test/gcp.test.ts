...
describe("gcp provider", () => {
  ...
  it("boots leases from the configured default snapshot", async () => {
    const client = new GCPClient({ ...env, CRABBOX_GCP_SNAPSHOT: "crabbox-dev-20260828" });
    (client as unknown as { cache: { token: string; expiresAt: number } }).cache = {
      token: "test-token",
      expiresAt: Math.trunc(Date.now() / 1000) + 3600,
    };
    ...
    client.fetcher = async (input, init) => {
      ...
      if (url.pathname.includes("/zones/us-central1-a/instances/crabbox-blue-lobster-")) {
        return Response.json({
          ...
          name: url.pathname.split("/").pop(),
          status: "RUNNING",
          machineType: "zones/us-central1-a/machineTypes/c4-standard-8",
          networkInterfaces: [{ accessConfigs: [{ natIP: "192.0.2.5" }] }],
        });
      }
      return Response.json({ name: "op-instance", status: "PENDING" });
    ...
    const disks = createCall?.body?.["disks"] as Array<Record<string, unknown>> | undefined;
    const params = disks?.[0]?.["initializeParams"] as Record<string, unknown> | undefined;
    // Unlike a machine image, a snapshot keeps the lease's own disk type and size.
    expect(params).toMatchObject({
      sourceSnapshot: "projects/default-project/global/snapshots/crabbox-dev-20260828",
      diskSizeGb: 120,
      diskType: "zones/us-central1-a/diskTypes/hyperdisk-balanced",
    });
    expect(params).not.toHaveProperty("sourceImage");
  });
